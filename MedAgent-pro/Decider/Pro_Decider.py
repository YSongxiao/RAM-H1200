...
class Pro_Decider:
    ...
    def extract_svdh_score(self, text: str):
    ...
    def decide_svdh_score(self, output_file: str, prompt: str, indicators, image_paths=None, field: str = "overall"):
        ...
        system_msg = concise_system(
            ...
            "Your job is to make a final Sharp/van der Heijde bone erosion score from 0 to 5."
        )
        user_prompt = concise_user(
            f"{prompt}\n\nIndicators and prior judgements:\n" + "\n".join(lines) + "\n\n"
            "Inspect the attached local joint patch and decide the final SvdH bone erosion score only. "
        ...
        score = self.extract_svdh_score(raw)
        result_obj = {
            "score": score,
            "raw": raw,
            "time_ms": elapsed_ms,
