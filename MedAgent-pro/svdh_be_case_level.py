...
from SvdH.tools import (
    build_svdh_scoring_context,
...
TOOL_FN_REGISTRY = {
    ...
    "build_svdh_scoring_context": build_svdh_scoring_context,
}
...
def run_quantitative_step(step, tool_by_id, plan_by_id, save_dir, image_path):
    ...
    for tid in tool_ids:
        ...
        if fn_name == "build_svdh_scoring_context":
...
def run_svdh_case(sample, task, plan, toolset, analyzer, decider, save_dir):
    ...
    question = create_svdh_be_prompt(sample)
...
def main():
    ...
    parser.add_argument("--data_root", type=str, default="SvdH")
    ...
    parser.add_argument("--output_dir", type=str, default="output/svdh_be_medagent_pro_case")
    ...
    output_path = os.path.join(args.output_dir, f"{args.model}_svdh_be_{args.split}.json")
    existing_output = load_json_snapshot(output_path, {})
    if (
        isinstance(existing_output, dict)
        and existing_output.get("task") == "svdh_be"
