...
EXP_PATHS = [
    # Each entry is an experiment directory containing a `test_metrics.csv`
    # produced by `main_score_cls.py --mode test` (BE or JSN score classification),
    # or a direct path to such a CSV file. Replace with your own runs.
    "./ckpts/Baseline_BEScore_be_resnet34_YYYYMMDDHHMMSS",
    "./ckpts/Baseline_JSNScore_New_jsn_resnet34_YYYYMMDDHHMMSS",
]
OUTPUT_PATH = "./score_metrics_summary.xlsx"
DECIMALS = 4
...
def infer_model_name(exp_dir: Path) -> str:
    ...
    if name.startswith("Baseline_JSNScore_jsn_"):
        ...
        return suffix.rsplit("_", 1)[0]
    if name.startswith("Baseline_JSNScore_New_jsn_"):
        suffix = name[len("Baseline_JSNScore_New_jsn_"):]
        return suffix.rsplit("_", 1)[0]
    if "_jsn_" in name:
        suffix = name.split("_jsn_", 1)[1]
        return suffix.rsplit("_", 1)[0]
    return name
