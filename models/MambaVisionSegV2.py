...
class _UNetDecoderBlock(nn.Module):
    def __init__(self, in_chans, skip_chans, out_chans, norm="bn"):
        super().__init__()
        self.fuse = nn.Sequential(
            _ConvModule(in_chans + skip_chans, out_chans, kernel_size=3, padding=1, norm=norm),
            _ConvModule(out_chans, out_chans, kernel_size=3, padding=1, norm=norm),
        )

    def forward(self, x, skip):
        x = _resize(x, size=skip.shape[-2:], align_corners=False)
        x = torch.cat([x, skip], dim=1)
        return self.fuse(x)


class _UNetDecoderHead(nn.Module):
    def __init__(self, encoder_channels, num_classes, decoder_channels=(512, 256, 128), norm="bn", dropout=0.1):
        super().__init__()
        self.encoder_channels = tuple(encoder_channels)
        self.bridge = nn.Sequential(
            _ConvModule(self.encoder_channels[-1], decoder_channels[0], kernel_size=3, padding=1, norm=norm),
            _ConvModule(decoder_channels[0], decoder_channels[0], kernel_size=3, padding=1, norm=norm),
        )
        self.decoder_blocks = nn.ModuleList(
            [
                _UNetDecoderBlock(decoder_channels[0], self.encoder_channels[-2], decoder_channels[1], norm=norm),
                _UNetDecoderBlock(decoder_channels[1], self.encoder_channels[-3], decoder_channels[2], norm=norm),
                _UNetDecoderBlock(decoder_channels[2], self.encoder_channels[-4], decoder_channels[2], norm=norm),
            ]
        )
        self.dropout = nn.Dropout2d(dropout) if dropout > 0 else nn.Identity()
    ...
    def forward(self, features):
        ...
        x = self.dropout(x)
        return self.classifier(x), decoder_features


class _MambaVisionFeatureExtractorV2(nn.Module):
    ...
    def load_pretrained_weights(self, checkpoint_path):
        ...
        print(
            f"MambaVisionV2 pretrained load: loaded {len(filtered_state)} tensors from {checkpoint_path}. "
...
class MambaVisionSegV2(nn.Module):
    def __init__(self, variant, in_chans, num_classes, image_size=224, pretrained=False, use_auxiliary=False):
        ...
        self.decode_head = _UNetDecoderHead(
    ...
    def forward(self, x):
        ...
        logits, decoder_features = self.decode_head(features)
        logits = F.interpolate(logits, size=input_size, mode="bilinear", align_corners=False)

        if not self.training or not self.use_auxiliary:
            return logits

        aux_logits = self.auxiliary_head(decoder_features[0])
...
def get_MambaVisionSegV2(variant, in_chans, num_classes, image_size=224, pretrained=False, use_auxiliary=False):
    return MambaVisionSegV2(
        variant=variant,
        in_chans=in_chans,
        num_classes=num_classes,
        image_size=image_size,
