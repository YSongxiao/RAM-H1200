    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for step, batch in enumerate(pbar):
                ...
                loss = self.criterion_G(None, final_pred, gt, 1)
                pred_bin = final_pred
                pred_bin[pred_bin > 0.5] = 1
                pred_bin[pred_bin <= 0.5] = 0
                dice_score_single = self.dice_metric(pred_bin, gt).squeeze().cpu().numpy().mean()
...
class TriSegTrainer:
    ...
    def train_one_epoch_amp(self, epoch):
        ...
        for step, batch in enumerate(pbar):
            ...
            if gt.device != self.device:
                gt = gt.to(self.device)
            if gt_sm.device != self.device:
                gt_sm = gt_sm.to(self.device)
            with autocast():
                pred, sm_pred, final_pred = self.net(img)
                if epoch < 30:
                    # loss = self.criterion(pred, final_pred, gt, gt_sm, 0.1)
                    loss = self.criterion(pred, sm_pred, final_pred, gt, gt_sm, False)
                else:
                    self.net.freeze_all_but_merger()
                    loss = self.criterion(pred, sm_pred, final_pred, gt, gt_sm, True)
                    # loss = self.criterion(pred, final_pred, gt, gt_sm, 1)
            self.scaler.scale(loss).backward()
            if self.grad_clip is not None:
                torch.nn.utils.clip_grad_norm_(self.net.parameters(), 1.0)
            self.scaler.step(self.optimizer)
...
class SegTester:
    ...
    def create_csv(self, args):
        ...
        metric_df = pd.concat(
         [fname_df, overlap_dsc_df, overlap_dsc_mean_df, overlap_nsd_df, overlap_nsd_mean_df, dsc_df, dsc_mean_df, nsd_df, nsd_mean_df, voe_df,
    ...
    def test(self):
        ...
        with torch.no_grad():
            for step, batch in enumerate(pbar):
                ...
                pred_bin[pred_bin > 0.5] = 1
                pred_bin[pred_bin <= 0.5] = 0
                # pred_overlap = pred_bin[:, 14:]
                # pred_bin = pred_bin[:, :14]

                # for num, (i, j) in enumerate(overlap_pairs):
                #     pred_bin[:, i, ...] = torch.logical_or(pred_bin[:, i, ...], pred_overlap[:, num, ...])
                #     pred_bin[:, j, ...] = torch.logical_or(pred_bin[:, j, ...], pred_overlap[:, num, ...])
